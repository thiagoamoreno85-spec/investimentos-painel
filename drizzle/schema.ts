...
/**
 * Aportes extraídos do extrato bancário
 */
export const cashDeposits = mysqlTable("cash_deposits", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  statementId: int("statementId").notNull(),
  description: varchar("description", { length: 256 }).notNull(), // Ex: "Transferência entrada", "Depósito"
  amount: decimal("amount", { precision: 18, scale: 2 }).notNull(),
  depositDate: timestamp("depositDate").notNull(),
  category: varchar("category", { length: 128 }).default("aporte").notNull(), // "aporte", "resgate", "outro"
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
